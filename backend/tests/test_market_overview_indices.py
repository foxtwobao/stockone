...
def test_latest_date_overview_keeps_indices_realtime(monkeypatch, tmp_path):
    seen = {}
    ...
    builder.build_market_overview(_FakeRepo(tmp_path), quote_service=_FakeQuoteService(), as_of=date(2026, 6, 29))

    assert seen["as_of"] is None


def test_historical_overview_keeps_indices_historical(monkeypatch, tmp_path):
    seen = {}
    ...
    builder.build_market_overview(_FakeRepo(tmp_path), quote_service=_FakeQuoteService(), as_of=date(2026, 6, 26))

    assert seen["as_of"] == date(2026, 6, 26)
